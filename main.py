...
class Translator:
    def translate_start(self, bot, update):
        get_info(update)
    ...
    def translate_stop(self, bot, update):
        get_info(update)
...
class Counter:
    def count_start(self, bot, update):
        get_info(update)
    ...
    def count_stop(self, bot, update):
        get_info(update)
...
def main(token):
    ...
    translate_handler = ConversationHandler(
        entry_points=[CommandHandler("translate", translator_handler.translate_start)],
        states={
            1: [MessageHandler(Filters.text, translator_handler.translate)]
        },
        fallbacks=[CommandHandler('stop', translator_handler.translate_stop)]
    )    
   
    calc_handler = ConversationHandler(
        entry_points=[CommandHandler("count", counter_handler.count_start)],
        states={
            1: [MessageHandler(Filters.text, counter_handler.count)]
        },
        fallbacks=[CommandHandler('stop', counter_handler.count_stop)]
    )      
   
    quotes_handler = ConversationHandler(
        entry_points=[CommandHandler("change_last_quotes", quotes.quotes_start, pass_user_data=True)],
        states={
            1: [MessageHandler(Filters.text, quotes.change_quotes, pass_user_data=True)]
        },
        fallbacks=[CommandHandler('stop', quotes.quotes_stop)]
    )      
    #Features:
